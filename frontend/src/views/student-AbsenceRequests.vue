<template>
  <IonPage>
    ...
    <IonContent>
      <div id="container">
        <div class="form-container">
          ...
          <div class="header-bottom">
            ...
            <IonTitle class="event-title">ABSENCE REQUEST</IonTitle>
          </div>

          <!-- Cards -->
          <div class="section-title">Absence Request Overview</div>
          ...
          <!-- Table -->
          <div class="section-title">Absence Request History</div>
          <div class="table-wrap">
            <table id="absenceTable" class="display styled-table" style="width:100%">
              <thead>
                <tr>
                  <th>Event Name</th>
                  <th>Date</th>
              ...
              <tbody>
                <tr v-for="row in rows" :key="row.absence_requests_id">
                  <td>{{ row.eventName }}</td>
                  <td>{{ row.date }}</td>
                  <td>
                    <span
      ...
      <!-- Modal for viewing absence request -->
      <ion-modal :is-open="isModalOpen" @didDismiss="closeModal" class="bootstrap-modal">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Absence Request Details</h5>
                    <button type="button" class="btn-close" @click="closeModal" aria-label="Close"></button>
                </div>
                <div class="modal-body" v-if="selectedRequest">
                    <p><strong>Student Name:</strong> {{ selectedRequest.student_name }}</p>
                    <p><strong>Department:</strong> {{ selectedRequest.department }}</p>
                    <p><strong>Year/Section:</strong> {{ selectedRequest.year_section }}</p>
                    <p><strong>Event Name:</strong> {{ selectedRequest.eventName }}</p>
...
const fetchAbsenceRequests = async () => {
 try {
  ...
  rows.value = res.data.map((r: AbsenceRequest) => ({
    ...r,
    eventName: r.event_name,
    date: formatDate(r.submission_date),
    status: r.status === 0
      ? "Pending"
      : r.status === 1
      ? "Approved"
...
async function onCancel(row: AbsenceRequest) {
  const result = await Swal.fire({
    title: 'Are you sure?',
    text: `Do you want to cancel the absence request for "${row.eventName}"?`,
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      ...
      Swal.fire({
        title: 'Cancelled!',
        text: 'Your absence request has been cancelled.',
        icon: 'success',
        didOpen: () => {
          document.body.classList.remove('swal2-height-auto');
          document.documentElement.classList.remove('swal2-height-auto');
...
function onView(row: AbsenceRequest) {
