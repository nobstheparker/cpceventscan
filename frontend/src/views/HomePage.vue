    ...
    <ion-content>
      <div id="container">
        ...
        <div class="bottom-content">
          ...
          <p>Sign up for volunteer opportunities!</p>
          <p>Foundation Day payments are due by March 5. Thank you.</p>
          <p>Good luck on your midterms! Don’t forget to study!</p>
        </div>
        <div class="footer">
...
const positionDropdown = (target: HTMLElement) => {
  ...
  if (targetLeft < containerWidth / 2) {
    ...
    dropdownPosition.value = {
      top: targetRect.bottom - containerRect.top + 6,
      left: targetLeft,
    };
  } else {
...
const dropdownPositionStyle = computed(() => {
  ...
  if (dropdownDirection.value === 'right') {
    ...
    return {
      top: `${dropdownPosition.value.top}px`,
      right: `${dropdownPosition.value.left}px`,
      left: 'auto',
    };
  }
});
...
const closeModal = () => {
  ...
  selectedEvent.value = null; // Reset selected event
};
...
const handleApplyClick = () => {
  ...
  const eventId = selectedEvent.value.id; // <-- add event ID

  setTimeout(() => {
    router.push({
      path: '/application-form',
      query: {
        id: eventId,       // include ID
        name: eventName,
        date: eventDate,
      },
    });
