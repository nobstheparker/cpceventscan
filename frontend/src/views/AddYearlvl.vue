<template>
  <ion-page>
    ...
    <ion-card class="year">
      ...
      <ion-card-header>
        <ion-card-title class="addyear">Add Year Level</ion-card-title>
      </ion-card-header>
      ...
      <ion-card-content>
        ...
        <ion-button expand="block" class="ion-addyear" @click="handleRegister">
          Add
        </ion-button>
...
const handleRegister = async () => {
  if (!selectedCourseId.value || !Yearlvl.value) {
  ...
  try {
    ...
    console.error('Failed to add year level', error);
    Swal.fire({
      icon: 'error',
      title: 'Error',
      text: 'Failed to add year level.',
      didOpen: () => {
        document.body.classList.remove('swal2-height-auto');
        document.documentElement.classList.remove('swal2-height-auto');
      }
...
ion-button.ion-addyear::part(native){
  background: #05044D;
}
